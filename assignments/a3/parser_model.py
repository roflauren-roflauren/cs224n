...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        """ 
        W and b1 in h = ReLU(xW + b1). h is 1 x |hidden nodes|. 
        then it follows: 
            x is 1 x dm so W must be dm x |hidden nodes|. 
            xW is 1 x |hidden nodes|, so b1 is 1 x |hiden nodes| too. 
        """
        self.embed_to_hidden_weight = nn.Parameter(torch.zeros(size=(self.embed_size * self.n_features, self.hidden_size)))
        self.embed_to_hidden_bias = nn.Parameter(torch.zeros(size=(1, self.hidden_size)))

        """
        U and b2 in l = hU + b2; l is 1 x |output classes|. 
        then it follows: 
            h is 1 x |hidden nodes| so U must be |hidden nodes| x |output classes|. 
            hU is 1 x |output classes|, so b2 is 1 x |output classes| too. 
        """
        self.hidden_to_logits_weight = nn.Parameter(torch.zeros(size=(self.hidden_size, self.n_classes)))
    ...
    def embedding_lookup(self, w):
        ...
        word_embeddings = self.embeddings[torch.flatten(w)]
        x = word_embeddings.view(torch.Size((w.shape[0], self.n_features, self.embeddings.shape[1])))
        ### END YOUR CODE
    ...
    def forward(self, w):
        ...
        hl_input = self.embedding_lookup(w).flatten(start_dim=1, end_dim=-1)
        h_raw = F.relu(torch.matmul(hl_input, self.embed_to_hidden_weight) + self.embed_to_hidden_bias)
        logits = torch.matmul(self.dropout(h_raw), self.hidden_to_logits_weight + self.hidden_to_logits_bias)    
        ### END YOUR CODE
