...
class PartialParse(object):
    def __init__(self, sentence):
        ...
        self.stack  = ["ROOT"]
        self.buffer = [token for token in sentence]
        self.dependencies = []
        ### END YOUR CODE
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while (len(unfinished_parses) > 0): 
        ...
        for partial_parse, predicted_transition in zip(minibatch, minibatch_predicted_transitions):
    ...
    # make sure 'dependencies' is in the same order as 'sentences':
    for idx in range(len(partial_parses)): 
