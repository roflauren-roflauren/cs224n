...
def naiveSoftmaxLossAndGradient(
    ...
    # create y_observed and y_hat distributions:
    pred_y, true_y = softmax(np.matmul(outsideVectors, centerWordVec)), np.zeros(outsideVectors.shape[0],)
    true_y[outsideWordIdx] = 1

    # y_hat - y_obs (and its transpose)
    diff_y           = np.subtract(pred_y, true_y) 
    diff_y_transpose = np.transpose(diff_y[:, None])

    # naiveSoftmax return updates:
    loss            = - np.log(pred_y[outsideWordIdx])
    gradCenterVec   = np.matmul(np.transpose(outsideVectors), diff_y)
    gradOutsideVecs = np.transpose(
                        np.matmul(
                            np.reshape(centerWordVec, (centerWordVec.size, 1)), diff_y_transpose)
                        )
    ### END YOUR CODE
...
def negSamplingLossAndGradient(
    ...
    word_vector_len = outsideVectors[0].size
    U_reformed = np.zeros(shape=(len(indices), word_vector_len))
    
    # nicely formed matrix of outside word vectors:
    for i, outside_idx in enumerate(indices): 
    ...
    # negSampling return updates:
    loss            = - np.sum(np.log(sig_Uv))
    gradCenterVec   = np.sum((U_reformed.T * sig_Uv_dec).T, axis = 0)
    gradOutsideVecs = np.zeros(shape=(len(outsideVectors), word_vector_len))
    for i, outside_idx in enumerate(indices):
        gradOutsideVecs[outside_idx] += -1 * sig_Uv_dec[i] * centerWordVec
        if i == 0: gradOutsideVecs[outside_idx] *= -1
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for i, word in enumerate(outsideWords): 
        # (curr outside word, center word) loss and grad: 
        ...
        # skipgram return updates: 
        loss                          += w_loss 
        gradCenterVecs[centerWordIdx] += w_gradCenterVec
